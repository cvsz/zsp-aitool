...
export function assertArtifactInsideOutputDir(outputDir: string, artifactPath: string): void {
  ...
  if (resolvedArtifactPath === resolvedOutputDir || !resolvedArtifactPath.startsWith(outputDirWithSep)) {
    throw new Error("ARTIFACT_OUTSIDE_OUTPUT_DIR");
  }
}
...
export async function resolveRenderArtifactPath(outputDir: string, outputPath: string, maxOutputMb?: number): Promise<string> {
  ...
  if (typeof maxOutputMb === "number" && maxOutputMb > 0) {
    const maxBytes = maxOutputMb * 1024 * 1024;
    if (artifactStat.size > maxBytes) throw new Error("ARTIFACT_TOO_LARGE");
  }
