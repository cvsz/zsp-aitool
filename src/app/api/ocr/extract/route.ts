...
export const POST = withAuth(async (request) => {
  try {
    ...
    if (!quota.allowed) return NextResponse.json({ error: "OCR request quota exceeded. Please retry later." }, { status: 429 });
    ...
    return NextResponse.json({ jobId: output.job.id, status: output.job.status, result: output.result, note: "โปรดตรวจสอบและแก้ไขผล OCR ก่อนบันทึกสินค้า" });
  } catch (error) {
